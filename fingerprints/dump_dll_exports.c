...
int WINAPI WinMain(HINSTANCE a,HINSTANCE b,LPSTR c,int d){
    ...
    /* BEGIN FINGERPRINTING */
   	char* fString = (char*)VirtualAlloc(NULL, 32000, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);
	HMODULE lib = LoadLibraryEx("<template>", NULL, DONT_RESOLVE_DLL_REFERENCES);
	...
	int index = 0;
	for (int i = 0; i < exports->NumberOfNames, index < data[1]; i++){
		// see if func exists
		BOOL is_normal_func = FALSE;
		for(int j = 0; j < sizeof(my_funcs)/sizeof(char*); j++){
			if(lstrcmpA((BYTE *)lib + (int)names[i], my_funcs[j]) == 0){
				is_normal_func = TRUE;
				break;
			}
		}
		if(!is_normal_func){
			size_t length =  mystrlen((BYTE *)lib + (int)names[i]);
			memcpymyas(fString+index, (BYTE *)lib + (int)names[i], length);
			memcpymyas(fString+index+length, "|", 1);
		  	index += length+1;
		}
